...
async def force_sub(bot, message):
    ...
    try:
       ...
       if member.status==enums.ChatMemberStatus.BANNED:
          await message.reply(f"😕 Dsl {message.from_user.mention}!\n Vous êtes temporairement exclu de notre chaîne, vous reviendrez bientôt")
       ...
       await message.reply(f"<b>🚫 Salut mon chèvre ami {message.from_user.mention}!\n\n Pour envoyer un message dans ce groupe, il faut d'abord être membre de notre communauté (ou chaîne) pour pouvoir communiquer ou récuperé vos fichiers  💯</b>", 
                       reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Rejoins la chaîne ✅", url=f_link)],
                                                          [InlineKeyboardButton("🌀 Je me suis abonné 🌀", callback_data=f"checksub_{message.from_user.id}")]]))
